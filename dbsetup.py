    ...
    with sqlite3.connect('questions.db') as conn:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS questions (
                ...
                answer_text TEXT,
                section_id INTEGER,
                file_path TEXT,  -- Для хранения пути к файлу
...
Рекомендуется включать английские субтитры, даже если в видео используется английский язык. https://technovationchallenge.org/ru/submission-guidelines/?utm_source=chatgpt.com""",
        ...
        (
            "Требования для технического видео",
            "Файл: ",
            "Сабмит",
            "files/techvideo2025.docx"
        ),
        (
            "Требования к видеопитчингу",
            "Файл:",
            "Сабмит",
            "files/pitchingrequirements.docx"
        ),
        (
            "Требования к сабмиту",
            "Файл:",
            "Сабмит",
            "files/teamsubmitrequirements.docx"
        ),
        (
            "Какие языки программирования можно использовать в проекте?",
            "Файл:",
            "Сабмит",
            "files/codinglanguage.docx"
        ),
        (
            "Могут ли команды представить что-то, помимо приложения?",
            "Это зависит от возраста. Команды младшего дивизиона могут представить: мобильное приложение, веб-приложение, Scratch проект. Команды старшего дивизиона могут представить: мобильное приложение, веб-приложение",
            "Сабмит",
            None
        ),
        (
            "План адаптации для младшего дивизиона",
            "Файл:",
            "Сабмит",
            "files/adoptationplan2025.docx"
        ),
        (
            "Бизнес план чек-лист",
            "Файл:",
            "Сабмит",
            "files/BusinessPlanChecklist2025.docx"
        ),
        (
            "Бизнес план шаблон",
            "Файл:",
            "Сабмит",
            "files/businessplantemplate.docx"
        ),
        (
            "Бизнес план пример",
            "Файл:",
            "Сабмит",
            "files/Business_Plan_EnGuard_GirlsEsc_upload.pdf"
        ),
        (
            "Рубрика младшего дивизиона",
            "Файл:",
            "Сабмит",
            "files/BeginnerRubric2024Season.pdf"
        ),
        (
            "Рубрика юниорского дивизиона",
            "Файл:",
            "Сабмит",
            "files/JuniorRubric2025.pdf"
        ),
        (
            "Рубрика старшего дивизиона",
            "Файл:",
            "Сабмит",
            "files/SeniorRubric2025.pdf"
        )
    ]
    ...
    with sqlite3.connect('questions.db') as conn:
        ...
        # Вставка вопросов
        for question_text, answer_text, section, file_path in questions_answers:
            section_id = section_ids.get(section)
            cursor.execute("SELECT 1 FROM questions WHERE question_text = ?", (question_text,))
            if cursor.fetchone() is None:
                # Вставка с путём к файлу (если путь есть)
                cursor.execute(
                    "INSERT INTO questions (question_text, answer_text, section_id, file_path) VALUES (?, ?, ?, ?)",
                    (question_text, answer_text, section_id, file_path)
                )
